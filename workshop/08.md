...
The FastAPI docs page will automatically populate your fields with values such as `str` for a string
or `0` for an integer. We can provide our own examples using `Field` and these will be used instead:

```python
offset_time: float | None = Field(None, examples=[60])
```
